...
TEST(StubsTest, EncodeClass) {
  zephir::config::StubsPtr stubs =
      std::make_shared<zephir::config::Stubs>("ide/%version%/%namespace%", false, "");
  ...
  EXPECT_EQ("ide/%version%/%namespace%", node["stubs"]["path"].as<std::string>());
